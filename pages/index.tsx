...
import {API_BASE_URL, DEFAULT_SYSTEM_PROMPT} from '@/utils/app/const';
...
const Home: React.FC<HomeProps> = ({serverSideApiKeyIsSet}) => {
    ...
    const handleKeyConfigurationValidation = (): boolean => {
        // 由于现在直接调用8080端口的服务器，不需要在前端配置API密钥
        // 服务器端会处理API密钥配置
        return true;
    }
    ...
    const handleSend = async (message: Message, deleteCount = 0) => {
        ...
        if (selectedConversation) {
            ...
            if (updatedConversation.index.indexName.length === 0) {
                ...
                response = await fetch(`${API_BASE_URL}/aichat/chat`, {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({
                        conversation_id: updatedConversation.id.toString(),
